		...
		if (isSuccess) {
			...
			if (redisVoteResponse.getResponseCount() >= Config.redisServerAmount) {
				...
				if (redisVoteResponse.getUserId() != null) {
					...
					Logger.info("Impossible to find the player " + redisVoteResponse.getUsername(), Logger.LogType.WARNING);
